...
import { Building2, Eye, Filter, X, Box, MapPin, Calendar, Ruler, Home, Shield, Waves, Dumbbell, Users, CheckCircle2, Sparkles, ArrowDown, Star, LayoutGrid } from 'lucide-react';
      ...
      <AnimatePresence>
        {isCompareMode && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="bg-sky-600 rounded-lg p-4 shadow-md"
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 bg-white/20 rounded-lg flex items-center justify-center">
                  <span className="text-lg font-bold text-white">{selectedForCompare.length}</span>
                </div>
                <div className="text-white">
                  <div className="font-semibold">דירות נבחרו להשוואה</div>
                  <div className="text-xs text-sky-100">עד 5 דירות</div>
                </div>
              </div>
              <div className="flex gap-2">
                <Button
